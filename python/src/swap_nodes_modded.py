...
if __name__ == '__main__':
    ...
    ifptr = open('/Users/yatin.kanetkar/misc_projects/sandbox/python/swap_nodes.in', 'r')

    n = int(next(ifptr).strip())

    
    indexes = []

    for _ in range(n):
        indexes.append(list(map(int, next(ifptr).rstrip().split())))

    queries_count = int(next(ifptr).strip())

    queries = []

    for _ in range(queries_count):
        queries_item = int(next(ifptr).strip())
